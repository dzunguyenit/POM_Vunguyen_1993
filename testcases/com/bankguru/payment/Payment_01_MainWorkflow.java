...
public class Payment_01_MainWorkflow extends AbstractTest {
	...
	public void TC_01_CreateNewCustomer() {
		...
		newCustomerPage.inputCustomerName(jsonData.payment().getCustomerName());
		log.info("Payment_01 Step 03 - Input Date Of Birth");
		newCustomerPage.inputDateOfBirth(jsonData.payment().getDateOfBirth());
		log.info("Payment_01 Step 04 - Input Address");
		newCustomerPage.inputAddress(jsonData.payment().getAddress());
		log.info("Payment_01 Step 05 - Input City");
		newCustomerPage.inputCity(jsonData.payment().getCity());
		log.info("Payment_01 Step 06 - Input State");
		newCustomerPage.inputState(jsonData.payment().getState());
		log.info("Payment_01 Step 07 - Input PIN");
		newCustomerPage.inputPIN(jsonData.payment().getPIN());
		log.info("Payment_01 Step 08 - Input Mobile Number");
		newCustomerPage.inputMobileNumber(jsonData.payment().getMobileNumber());
		...
		newCustomerPage.inputPassword(jsonData.payment().getPasswordCustomer());
